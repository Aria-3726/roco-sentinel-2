...
export default function App() {
  ...
  return <main>
    ...
    <div className="wrap">
      ...
      {tab === "execution" && <><div className="grid metrics">{(roster.by_list_type || []).map((item,index)=><Metric key={item.name} label={TYPE[item.name]||item.name} value={fmt(item.count)} note="已导入私有名单" tone={["blue","purple","orange","green"][index%4]}/>) }<Metric label="表内已填发布链接" value={fmt(roster.published_creators)} note="草稿与排期不计发布" tone="green"/></div><div className="notice green"><span>🔒</span><div><strong>商业数据保持私有</strong><p>报价、联系人、邮箱、Discord 与供应商备注仅存于数据库，不通过公开接口或网页返回。</p></div></div><div className="grid two"><Card><div className="head"><h2>建联与执行状态</h2><p>仅展示聚合数量</p></div><div className="pad bars">{(roster.by_status||[]).map(item=><div key={item.name}><div className="barLabel"><span>{item.name}</span><b>{fmt(item.count)}</b></div><div className="track"><div className="fill" style={{width:`${Math.max(2,Number(item.count)/statusTotal*100)}%`}}/></div></div>)}</div></Card><Card><div className="head"><h2>可监测平台账号</h2><p>一位创作者可对应多个平台</p></div><div className="list">{(roster.by_platform||[]).map(item=><div className="listRow" key={item.name}><strong>{PLATFORM[item.name]||item.name}</strong><span className="muted">账号</span><Chip color={PLATFORM_COLOR[item.name]}>{TYPE.koc} / KOL / 媒体</Chip><b>{fmt(item.count)}</b></div>)}</div></Card></div></>}
